...
/// A short human label for the generated spec's provenance header
/// (`# generated by deadreckon detect: <label>`) and preview text.
pub fn kind_label(kind: &ProjectKind) -> String {
    match kind {
        ProjectKind::Rust => "rust".to_string(),
        ProjectKind::Node(pm) => format!("node ({})", package_manager_program(*pm)),
        ProjectKind::Deno => "deno".to_string(),
        ProjectKind::Go => "go".to_string(),
        ProjectKind::Python => "python".to_string(),
        ProjectKind::Elixir => "elixir".to_string(),
        ProjectKind::Dotnet => "dotnet".to_string(),
        ProjectKind::Jvm(BuildTool::Maven) => "jvm (maven)".to_string(),
        ProjectKind::Jvm(BuildTool::Gradle) => "jvm (gradle)".to_string(),
        ProjectKind::Jvm(BuildTool::GradleWrapper) => "jvm (gradle wrapper)".to_string(),
        ProjectKind::Ruby(_) => "ruby".to_string(),
        ProjectKind::Php(_) => "php".to_string(),
        ProjectKind::ScriptRunner(Runner::Make) => "script-runner (make)".to_string(),
        ProjectKind::ScriptRunner(Runner::Just) => "script-runner (just)".to_string(),
        ProjectKind::ScriptRunner(Runner::Task) => "script-runner (task)".to_string(),
        ProjectKind::Unknown => "unknown".to_string(),
    }
}
