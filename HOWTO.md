...
### Non-interactive def-done (GUI and scripts)

Machine callers declare, read back, and dry-run the contract without a TTY. `--dir <path>` names the project that owns `.deadreckon/` (default: the current directory), `--json` makes stdout exactly one JSON envelope, and `--yes` supplies the approval the interactive flow would have collected — declaring or adding with `--json` but without `--yes` refuses:

```bash
deadreckon def-done "build, test, and open in a browser" --yes --json --dir ~/code/app
deadreckon def-done add "users can save drawings" --yes --json --dir ~/code/app
deadreckon def-done show --json --dir ~/code/app
deadreckon def-done check --json --dir ~/code/app
```

Success emits a `def_done_result` envelope: `{kind:"def_done_result", status, contract_path, notes_path, name, check_count, checks:[…], capabilities:{network}, drafted_by, next_actions}`. `checks` is the same serialized `AcceptanceSpec` check shape `report --json` emits (tagged `kind`, `must_pass`, and the kind-specific fields), so an app never parses YAML. `status` reads `written` after declare/add, `declared`/`default_gate` from show (a missing contract is a normal read, exit 0), and `passed` from check, which adds per-check `results`. Drafting still calls the configured provider; every failure — provider errors, critic rejection, a required check failing during `check` — lands as the shared `{kind:"error", code, verb:"def-done", message, try_lines}` refusal envelope on stdout with the historical exit code.
