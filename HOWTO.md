...
`setup` and `add` ask the configured provider to convert plain-English criteria into `.deadreckon/acceptance.yaml`. Common packs are available without a provider:

```bash
deadreckon acceptance add build
deadreckon acceptance add node
deadreckon acceptance add static-site
deadreckon acceptance add browser
deadreckon acceptance add playwright
deadreckon acceptance add vite
deadreckon acceptance add nextjs
deadreckon acceptance add python
```
