...
**As built (Lay Course follow-up), non-interactive `def-done`:** the Lay Course sheet's "Draft with LLM — you approve" button and the `deadreckon def-done "…" && deadreckon start …` try-line now work without a TTY. `def-done` takes `--dir <path>` (the project owning `.deadreckon/`; default cwd — the sheet passes the repo it is laying a course in), `--json` (one envelope on stdout, G1-armed so every refusal is the shared `{"kind":"error","verb":"def-done",…}` object), and `--yes` (the approval the interactive flow would have collected; declare/add/edit with `--json` but without `--yes` is a typed refusal whose `try_lines` carry the corrected command). Success emits `{"kind":"def_done_result","status","contract_path","notes_path","name","check_count","checks":[…],"capabilities":{"network"},"drafted_by","next_actions"}` — `checks` is the exact serialized `AcceptanceSpec` shape `report --json` already emits (tagged `kind` + `must_pass` + kind-specific fields), so CONTRACTS.md's no-app-side-YAML rule holds; `drafted_by` names the drafting route (`"<provider> / <model>"`, or `"<pack> pack"` for provider-free pack adds). Verb map: declare/`add`/`edit` → `status:"written"` (drafting still calls the provider — the app shows progress; provider/critic failures arrive as typed refusal envelopes); `show --json` → `"declared"`, or `"default_gate"` with `contract_path:null` and empty `checks` when nothing is declared (a normal read, exit 0 — the sheet renders the declare affordance); `check --json` → `"passed"` plus per-check `results` (`AcceptanceCheckResult` rows), while a required failure keeps the fail-closed surface as an error envelope with exit 1. The sheet's launch leg needs no addition: with the contract declared in the project, `start "<goal>" --from <dir> --json` from that project previews launchable (`will_start:false`, `verdict.kind:"preview"`) and the preview's `done_contract` block already carries the resolved compiled checks and granted capabilities (pinned end-to-end in tests/lifecycle.rs `done_declare_json_then_start_from_preview_is_launchable`). Trust boundary unchanged: this surface only lets the operator's own app do the declaring it could already do at a TTY — dr-gate, freezing, and promotion are untouched, and the contract remains operator-approved (`--yes`) before any launch freezes it. Two cross-layer fixes ride along: `start --from <dir>` roots the done contract at `<dir>` (not the caller's cwd — a GUI child runs from `$HOME`, and cwd-rooted resolution left the declare→re-preview flow permanently blocked, or worse, silently used a `$HOME` contract), and `scripts/vendor-cli.sh` now vendors + signs `dr-gate` next to the CLI (same cargo invocation, same bundle build id; pinned as `gateSha256` in manifest.json) because the execute leg's job admission requires the trusted helper beside the running binary — `release-app.sh` fails closed without it.
