...
digest; children never work from the mutable external path. Goal-shape
planning, draft, critic and optional redraft use schema-only provider requests
under one 1,200-second default automated admission-work allowance (or an earlier
explicit Job deadline). Each automated phase inherits the same remaining time;
operator prompts pause that work clock, but an explicit calendar deadline does
not. Recovery is failure-typed: semantic rejection retains the candidate and
exact findings for revision, exhausted work requires an explicitly authorized
new bounded attempt, and an expired calendar deadline offers no retry. Shape
planning also retains its route cap. Cancellation reaps the full provider
process tree, and admission stops if the separate 30-second cleanup window
cannot prove the provider exited.
...
| Definition-of-done compiler and admission | Make acceptance executable instead of conversational | `def-done` and setup infer or generate the independent gate command; guided authoring keeps files at the launch root while a capped/redacted dossier describes the resolved source, uses strict output schemas under one 1,200-second cumulative automated-work allowance shared with route-capped goal-shape planning (operator prompts pause it; an explicit calendar deadline does not), preserves rejected candidates and exact findings for typed recovery, and durable admission atomically refuses empty, optional-only and working-directory-only contracts before a Job exists | Converts real source facts into a portable machine-checkable boundary without giving the authoring provider tools or an unbounded pre-launch turn | **Stable / bounded** | **Essential; extend** with rules-as-gate and clearer explanations |
...
| Durable launch and control | `run`, `orchestrate`, `campaign`, `chain`, `supervisor`, `steer`, `kill`, `extend` | Supported non-preview launches enter Jobs except explicit in-place/uncontained `run`; `steer` works for any executing run (delivered mid-turn on the Codex app-server route, consumed at the next turn boundary on every other provider) and does not target a Job backing run. |
