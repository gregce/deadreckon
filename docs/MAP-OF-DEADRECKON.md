...
optional redraft use schema-only provider requests under one 900-second default
deadline, reserve time for later contract stages and reap the full provider
process tree on cancellation. Goal-shape planning is separately bounded at 120
seconds for CLI providers and 30 seconds for HTTP providers; admission stops if
the following 30-second cleanup window cannot prove the provider exited.
...
| Definition-of-done compiler and admission | Make acceptance executable instead of conversational | `def-done` and setup infer or generate the independent gate command; guided authoring keeps files at the launch root while a capped/redacted dossier describes the resolved source, uses strict output schemas under one 900-second default budget with reserved later stages, and durable admission atomically refuses empty, optional-only and working-directory-only contracts before a Job exists | Converts real source facts into a portable machine-checkable boundary without giving the authoring provider tools or an unbounded pre-launch turn | **Stable / bounded** | **Essential; extend** with rules-as-gate and clearer explanations |
