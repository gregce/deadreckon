...
not. No phase receives a fresh clock or a divided share. Cancellation reaps the full provider process tree,
and admission stops if the separate 30-second cleanup window cannot prove the
provider exited.
...
| Definition-of-done compiler and admission | Make acceptance executable instead of conversational | `def-done` and setup infer or generate the independent gate command; guided authoring keeps files at the launch root while a capped/redacted dossier describes the resolved source, uses strict output schemas under one 1,200-second cumulative automated-work allowance shared with goal-shape planning (operator prompts pause it; an explicit calendar deadline does not), and durable admission atomically refuses empty, optional-only and working-directory-only contracts before a Job exists | Converts real source facts into a portable machine-checkable boundary without giving the authoring provider tools or an unbounded pre-launch turn | **Stable / bounded** | **Essential; extend** with rules-as-gate and clearer explanations |
