...
Provider-backed narration is an overlay on these projections. Manual `r` refresh and TTY narrative-view event refreshes build a redacted prompt from the deterministic projection, send only bounded evidence summaries to the selected provider route (`--narrative-provider`, then run/plan provider fallback), validate all returned claims against known evidence ids, reject invented graph ids, and persist a new fresh snapshot only after validation. Event refreshes can bypass the ordinary freshness interval for meaningful deltas, but budget, provider availability, JSON validation, citation validation, graph validation, and redaction guards still apply. If the provider is missing, over budget, behind cadence, returns malformed JSON, cites unknown evidence, emits secret-like text, or fails, attach keeps running and persists a stale deterministic snapshot with the error in `state.json`.
